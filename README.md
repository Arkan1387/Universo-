...
<body class="flex flex-col min-h-screen">
    ...
    <header class="bg-slate-900 p-4 border-b border-slate-800 flex justify-between items-center sticky top-0 z-50">
        <h1 class="font-black text-white uppercase text-xs tracking-widest italic">Protocolo Ω V2.3</h1>
        <div class="flex gap-2">
            ...
            <button onclick="exportData()" class="bg-slate-700 text-[10px] px-2 py-1 rounded font-bold text-white">💾</button>
        </div>
    </header>
    ...
    <script>
        const PREFIX = 'v11_';
        const SCHEMA = [
            { id: 'inicio', title: '🏠Inicio', subs: [] },
            { id: 'mundo', title: '🌐Sistema de Mundo', subs: ['Jerarquias', 'Megaplaneta Ω', 'Leyes del Nyra y Alma', 'Habilidades Nyricas', 'Dimensiones', 'Planetas Eje', 'Sistemas', 'Reino Divino', 'Deseos'] },
            { id: 'crono', title: '⌛Cronología', subs: ['Pasado', 'Hitos Actuales', 'Importantes'] },
            { id: 'notas', title: '📘Notas', subs: ['Ideas', 'Lluvia de Ideas', 'Dudas'] },
            { id: 'geo', title: '🗺️Geografía', subs: ['Mapas/Clima', 'Ciudades', 'Continentes'] },
            { id: 'caps', title: '📰Capítulos', subs: ['Estructura', 'Resúmenes', 'Progreso'] },
            { id: 'escena', title: '📄Escena Actual', subs: ['Borrador', 'Diálogos', 'Revisiones'] },
            { id: 'arte', title: '⚔️Artefactos', subs: ['Mágicos', 'Tecnológicos', 'Divinos'] },
            { id: 'bestia', title: '🐲Animales / Monstruos', subs: ['Depredadores', 'Domésticos', 'Míticos'] }
        ];
        ...
        function init() {
            ...
            SCHEMA.forEach((sec, idx) => {
                ...
                if (sec.subs.length > 0) {
                    ...
                    subHtml = `<textarea id="data-${sec.id}" oninput="saveRaw('${sec.id}')" class="h-96" placeholder="Escribe aquí la premisa o el resumen general de tu universo..."></textarea>`;
                }
                ...
                container.innerHTML += `
                    <div id="${sec.id}" class="page-content ${idx===0?'active':''}">
                        ...
                        <div class="mb-8 p-4 bg-slate-900/50 rounded-xl border border-slate-800">
                            <p class="text-[9px] text-slate-500 uppercase font-bold mb-3 italic">🖼️ Ilustración General del Apartado</p>
            ...
            (db_entries[key] || []).forEach(entry => {
                target.innerHTML += `
                    <div class="entry-card">
                        ...
                        <textarea oninput="updateEntry('${key}', ${entry.id}, 'text', this.value)" class="min-h-[160px] mb-4" placeholder="Escribe el conocimiento aquí...">${entry.text}</textarea>
                        ...
                        <div class="bg-black/30 p-2 rounded border border-slate-800">
                             ...
                             <input type="text" oninput="updateEntry('${key}', ${entry.id}, 'links', this.value)" value="${entry.links}" placeholder="Ej: Arkan, Ciudad de Ω, Reliquia..." class="bg-transparent text-[10px] text-sky-400 outline-none w-full italic">
                        </div>
                    </div>`;
            });
        }
